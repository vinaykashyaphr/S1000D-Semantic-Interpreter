...
struct Interpreter::Impl {
    ...
    void transform_glyphs(std::string& xml_content, const std::unordered_map<std::string, std::string>& glyphs ) {
...

{
    ...
    _impl->transform_glyphs(xml_content, GLYPHS_MAP);
    _impl->parse(xml_content);
    _impl->interpret();
...
BaseModel* Interpreter::get_model_base(std::string_view xpath) {
    return _impl->_registry.get_model<BaseModel>(xpath, _impl->doc_root());
}
